            ...
            match circuit_state.get(&message_len_in_bits) {
                None => {
                    ...
                    let output_bytes = timed!(
                        timing,
                        "serialize proof and verifier data for aggregation",
                        {
                            let deserialized_proof = proof.to_bytes();
                            let deserialized_verifier_data =
                                circuit_data.0.verifier_only.to_bytes().unwrap();
                    ...
                    timing.print();
                    client
                        .publish("PROCESS_SIGNATURE_RESULT".to_string(), output_bytes.into())
                        .await?;
                }
                Some(hashed_circuit) => {
                    ...
                    let output_bytes = timed!(
                        ...
                        "serialize proof and verifier data for aggregation",
                        {
                            let deserialized_proof = proof.to_bytes();
                            let deserialized_verifier_data =
                                deref_circuit_data.0.verifier_only.to_bytes().unwrap();
                            let output = OutputTask {
                                proof: deserialized_proof,
                                verifier_data: deserialized_verifier_data,
                                signature_index: payload.signature_index,
